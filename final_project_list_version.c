...
int hot_spot_visited_function(const void *data);
...
void hot_spots(){
    ...
    int (*temp_function_pointer)(const void *data) = place_list->place_list->print_func;
    place_list->place_list->print_func = hot_spot_visited_function;
    place_list->place_list->print_list_from_head(place_list->place_list);
    place_list->place_list->print_func = temp_function_pointer;
...
void delete(){
    ...
    while (1){
        ...
        search_result = student_list->student_list->search_node_from_head(student_list->student_list, &search_target);
...
void print_usage(){
    printf("Usage : ./linked_list_ver [option]\n");
    printf("option :\n");
...
void print_out_potential_contacts(place_record *record){
    ...
    struct place *place_search_reult = place_list->place_list->search_node_from_head(place_list->place_list, &place_target);
...
void search_place_id(){
    while (1){
        ...
        struct place *search_place_result = place_list->place_list->search_node_from_head(place_list->place_list, &place_target);

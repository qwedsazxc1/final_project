...
static int push(struct stack *stack, int data){
    int push_result = stack->stack_list.add_node_to_head(&(stack->stack_list.head), data);
    if (push_result == -1){
...
static int pop(struct stack *stack){
    ...
    stack->stack_list.delete_node(&(stack->stack_list.head), stack->top(*stack));
    stack->stack_size -= 1;
    stack->stack_top = stack->top(*stack);
    return 0;
}
